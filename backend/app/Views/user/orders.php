...
<style>
    ...
    .tab-btn.active { border-bottom-color: #E87722; color: #E87722; }
    .order-card     { transition: all 0.3s ease; }
    .order-card:hover { box-shadow: 0 8px 32px rgba(59,35,20,0.10); transform: translateY(-2px); }
    .fiber-bg {
    ...
    /* Timeline */
    .timeline-step.done   .dot { background: #4A7C59; }
    .timeline-step.active .dot { background: #E87722; box-shadow: 0 0 0 4px rgba(232,119,34,0.2); }
    .timeline-step.idle   .dot { background: #EDE0CC; }
    .timeline-line.done        { background: #4A7C59; }
    .timeline-line.idle        { background: #EDE0CC; }
</style>
...
<div id="order-modal" class="fixed inset-0 z-[9997] hidden items-center justify-center p-4" onclick="if(event.target===this)closeModal()">
    <div class="absolute inset-0 bg-coco-brown/40 backdrop-blur-sm"></div>
...
<script>
// Demo order data for modal (in production, fetch from API)
const ordersData = <?= json_encode(array_merge(...array_values($grouped))) ?>;
...
function showOrderDetail(orderId) {
    ...
    const labelMap = { to_pay:'Awaiting Payment', to_ship:'Processing', to_receive:'Out for Delivery', completed:'Completed', refund:'Refund' };

    let itemsHtml = (order.items || []).map(item => `
        <div class="flex items-center gap-3">
            <img src="/images/${item.product_image}" class="w-12 h-12 rounded-xl object-cover bg-gray-100">
            <div class="flex-1">
                <div class="font-bold text-sm text-coco-brown">${item.product_name}</div>
    ...
    document.getElementById('modal-body').innerHTML = `
        <div class="flex items-center gap-2 mb-2">
            <span class="text-xs ${badgeMap[order.status]} font-bold rounded-full px-3 py-1">${labelMap[order.status]}</span>
        ...
        <div class="border-t border-coco-sand/40 pt-4 space-y-2 text-sm">
            ...
            <div class="flex justify-between"><span class="text-coco-mid">Shipping</span><span class="font-bold">${order.shipping > 0 ? '₱'+Number(order.shipping).toLocaleString('en-PH',{minimumFractionDigits:2}) : 'FREE'}</span></div>
            <div class="flex justify-between text-base"><span class="font-bold text-coco-brown">Total</span><span class="font-black text-coco-orange text-xl">₱${Number(order.total_amount||0).toLocaleString('en-PH',{minimumFractionDigits:2})}</span></div>
        </div>
        <div class="bg-coco-sand/30 rounded-2xl p-4 space-y-1 text-xs text-coco-mid">
            <div><strong class="text-coco-brown">Payment:</strong> ${payLabel}</div>
            <div><strong class="text-coco-brown">Receive via:</strong> ${receiveLabel}</div>
        </div>
        ${order.status === 'to_receive' ? `
        <button class="w-full bg-coco-green text-white font-bold py-3 rounded-full hover:bg-coco-dark transition-colors" onclick="closeModal()">
            ✓ Confirm Order Received
        </button>` : ''}
        ${order.status === 'completed' ? `
        <button class="w-full border-2 border-red-200 text-red-500 font-bold py-3 rounded-full hover:bg-red-50 transition-colors text-sm" onclick="closeModal()">
